...
export const getBook = () => async (dispatch, getState) => {
  let res;
  try {
    res = await axios.post(`${process.env.REACT_APP_API_URL}/book/allbook`, {
...
export const addAuthor = (name) => async (dispatch, getState) => {
  ...
  let res;
  try {
    res = await axios.post(`${process.env.REACT_APP_API_URL}/admin/addauthor`, {
      name: name,
    });
  } catch (err) {
...
export const addBook =
  (id_category, name, price, release_date, describe, id_nsx, id_author, file) =>
...
export const getBill = (status) => async (dispatch, getState) => {
  ...
  if (status === "0") {
    link = `${process.env.REACT_APP_API_URL}/bill/status/0`;
  }
  if (status === "1") {
    link = `${process.env.REACT_APP_API_URL}/bill/status/1`;
  }
  let res = null;
  try {
